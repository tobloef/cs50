    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if (requestLine[1] == NULL) {
                error(400);
                continue;
            } else if (requestLine[1][0] != '/' || strchr(requestLine[1], '.') != NULL) {
                error(501);
                continue;
            ...
            if (requestLine[2] == NULL ||
            	strcmp("HTTP/1.1", requestLine[2]) != 0) {
                error(505);
                continue;
            }

            char* qi = strchr(requestLine[1], '?');
            char* eos = strchr(requestLine[1], '\0');
            char query[(eos - qi < 1) ? 1 : eos - qi];
            memcpy(
                query, 
                (strncmp(qi, "\0", 1) == 0) ? qi : qi + 1, 
                (eos - queryInd < 1) ? 1 : eos - qi
            );

            qi = strchr(requestLine[1], '?');
            char absolutePath[qi - requestLine[1] + 1];
            strncpy(
                absolutePath, 
                requestLine[1], 
                queryInd - requestLine[1]
            );
            absolutePath[qi - requestLine[1]] = '\0';
            
            char* ei = strchr(absolutePath, '.');
            if (ei == NULL) {
                error(501);
                continue;
            }

            ei = strchr(absolutePath, '\0');
            char extension[eos - ei];
            memcpy(extension, ei + 1, eos - ei);
